...
#
# v1.5 Capability-Enforced DAG Validator.
#
# This validator enforces ONLY structural correctness of directed acyclic graphs
# PLUS v1.5 capability manifest validation.
#
# ================================================================
...
from core.tool_contracts import get_contract, get_required_capabilities_from_contract
...
class DAGValidator:
    ...
    def validate(self, dag: DAG) -> Dict[str, Any]:
        ...
        errors.extend(self._validate_capability_manifest(dag))
    ...
    def _validate_capability_manifest(self, dag: DAG) -> List[str]:
        """
        Validate the v1.5 capability manifest on a DAG.

        Checks:
        - manifest exists
        - strict_mode is True
        - every tool in the DAG has required capabilities declared in the manifest
        - no capability escalation (tool requires capability X but manifest grants Y)
        """
        errors: List[str] = []

        manifest = getattr(dag, "capability_manifest", None)
        if manifest is None:
            errors.append("Capability manifest is missing from DAG — execution blocked")
            return errors

        if not getattr(manifest, "strict_mode", False):
            errors.append("Capability manifest must have strict_mode=True for v1.5")
            return errors

        manifest_cap_ids: Set[str] = set()
        for cap in getattr(manifest, "capabilities", []):
            ...
            if cid:
                manifest_cap_ids.add(str(cid))

        for node in dag.nodes:
            tool_name = node.tool_name
            tool_caps = self._get_tool_required_capabilities(tool_name)
            for req_cap in tool_caps:
                if req_cap not in manifest_cap_ids:
                    errors.append(
                        f"Node {node.node_id}: tool '{tool_name}' requires "
                        f"capability '{req_cap}' but it is not granted in manifest"
                    )

        return errors

    def _get_tool_required_capabilities(self, tool_name: str) -> List[str]:
        if not tool_name:
            return []
        try:
