...
class ToolContract:
    ...
    required_capabilities: List[str] = field(default_factory=list)  # v1.5 e.g. ["filesystem.write"]
...
_TOOL_CONTRACTS: Dict[str, ToolContract] = {
    "write_file": ToolContract(
        ...
        required_capabilities=["filesystem.write", "tool.execution"],
    ),
    "read_file": ToolContract(
        ...
        required_capabilities=["filesystem.read", "tool.execution"],
    ),
    "list_dir": ToolContract(
        ...
        deterministic=True,
        side_effects=["filesystem_read"],
        required_capabilities=["filesystem.list", "tool.execution"],
    ),
...
def get_required_capabilities_from_contract(name: str) -> List[str]:
    """Get the list of required capability IDs for a tool contract."""
    if name not in _TOOL_CONTRACTS:
        raise ValueError(f"Unknown tool contract: {name}")
    return list(_TOOL_CONTRACTS[name].required_capabilities)
