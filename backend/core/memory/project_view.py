...
class ProjectIntelligenceView:
    def __init__(
        ...
        self._change_lens = ChangeLens(store=change_store)
        self._relationship_lens = RelationshipLens(store=relationship_store)
        self._activity_lens = WorkspaceActivityLens(tracker=activity_tracker)
    ...
    def changes(self) -> Dict[str, Any]:
        timeline = self._change_lens.timeline(count=50)
        component_activity = self._change_lens.component_evolution()
        goal_activity = self._change_lens.goal_evolution()
        return {
            "lens": "changes",
            "timeline": timeline,
    ...
    def relationships(self) -> Dict[str, Any]:
        file_rels = self._relationship_lens.file_relationships()
        component_rels = self._relationship_lens.component_relationships()
        goal_rels = self._relationship_lens.goal_relationships()
        return {
            "lens": "relationships",
            "file_relationships": file_rels,
            "component_relationships": component_rels,
            "goal_relationships": goal_rels,
        }

    def workspace(self) -> Dict[str, Any]:
        return {
            "lens": "workspace",
            "recent_activity": self._activity_lens.recent_activity(hours=24, limit=50),
            "active_files": self._activity_lens.active_files(limit=20),
            "workspace_summary": self._activity_lens.workspace_summary(),
        }
