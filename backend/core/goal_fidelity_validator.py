...
class GoalFidelityValidator:
    ...
    def validate(
        ...
        non_synthetic = [n for n in dag.nodes if not n.metadata.get("synthetic")]
        if len(non_synthetic) != len(steps):
            errors.append(
                f"DAG node count ({len(non_synthetic)} non-synthetic) "
                f"!= structured steps ({len(steps)})"
            )
