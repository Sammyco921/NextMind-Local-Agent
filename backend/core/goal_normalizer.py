...
class GoalNormalizer:
    ...
    def _guess_type(text: str) -> str:
        ...
        if any(w in lower for w in ("create", "write", "save", "file", "document")):
            return "write_file"
        if any(w in lower for w in ("read", "open")):
            return "read_file"
    ...
    def _extract_entities(self, text: str, type_guess: str) -> Dict[str, Any]:
        ...
        if called_file:
            fname = called_file.group(1).strip()
            ...
            if not entities.get("filename"):
                ...
                entities.setdefault("filenames", []).append(fname)
    ...
    def _extract_content(text: str) -> Optional[str]:
        ...
        for pattern in (
            r"(?:exact\s+)?contents?\s*:\s*(.+)$",
            r"with\s+(?:the\s+)?(?:exact\s+)?contents?\s*:\s*(.+)$",
            r"with\s+contents?\s+(.+)$",
            r"contents?\s+(.+)$",
        ):
