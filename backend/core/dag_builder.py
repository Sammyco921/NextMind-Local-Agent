...
_KNOWN_TOOLS = frozenset({"write_file", "read_file", "list_dir", FAILURE_INJECTION_TOOL})
...
_SYNTHETIC_NOOP_TOOLS = frozenset({"read_file", FAILURE_INJECTION_TOOL})
...
class DAGBuilder:
    ...
    def build(
        ...
        synthetic_nodes: List[DAGNode] | None = None,
    ) -> DAGBuildResult:
        if not steps:
        ...
        warnings: List[str] = []
        prepared, prep_warnings, prep_errors = self._prepare_steps(steps, spec=spec)
        ...
        if synthetic_nodes:
            merged = self._inject_synthetic(nodes, synthetic_nodes, warnings)
            if merged is None:
                return DAGBuildResult(
                    dag=DAG(
                        nodes=[structured_error_node(
                            "Synthetic node injection failed: unknown dependency target",
                            phase="dag_build",
                        )]
                    ),
                    status="error",
                    errors=["Synthetic node injection failed"],
                    warnings=warnings,
                    structured_steps=with_deps,
                )
            nodes = merged
    ...
    def _prepare_steps(
        ...
        allowed_tools = spec.allowed_tools if spec is not None else _KNOWN_TOOLS
        warnings: List[str] = []
        errors: List[str] = []
    ...
    # ---- Synthetic node injection (v1.6 ExecutionMode) ----

    @classmethod
    def _inject_synthetic(
        cls,
        nodes: List[DAGNode],
        synthetic: List[DAGNode],
        warnings: List[str],
    ) -> List[DAGNode] | None:
        """Inject synthetic nodes into the DAG with topological validity."""
        valid_ids = {n.node_id for n in nodes}
        result = list(nodes)

        for syn_node in synthetic:
            syn_deps = syn_node.dependencies
            for dep in syn_deps:
                if dep not in valid_ids:
                    warnings.append(
                        f"Synthetic node {syn_node.node_id} depends on unknown "
                        f"node '{dep}' — injection failed"
                    )
                    return None

            result.append(syn_node)
            valid_ids.add(syn_node.node_id)
    ...
    def make_failure_node(
        ...
        failure_type: str = "capability_violation",
    ) -> DAGNode:
        return DAGNode(
            node_id=node_id,
            tool_name=FAILURE_INJECTION_TOOL,
            raw_args={"failure_type": failure_type},
            dependencies=sorted(dependencies),
            metadata={"synthetic": True, "failure_node": True, "failure_type": failure_type},
        )

    @classmethod
    def make_noop_identity_node(
        cls,
        node_id: str,
        dependencies: List[NodeId],
        filename: str,
    ) -> DAGNode:
        """Create a no-op read_file node for STRESS_TEST expansion."""
        return DAGNode(
            node_id=node_id,
            tool_name="read_file",
            raw_args={"filename": filename},
            dependencies=sorted(dependencies),
            metadata={"synthetic": True, "noop_identity": True},
        )

    @classmethod
    def compute_stress_noops(
        cls,
        nodes: List[DAGNode],
        extra_count: int,
    ) -> List[DAGNode]:
        """Deterministically generate no-op nodes for STRESS_TEST expansion.

        Each no-op reads from a previous node's output via artifact reference.
        Uses sorted node IDs for deterministic iteration.
        """
        sorted_nodes = sorted(nodes, key=lambda n: n.node_id)
        synthetic: List[DAGNode] = []
        next_id = len(sorted_nodes)

        for i in range(extra_count):
            dep_idx = next_id - 1
            source = sorted_nodes[dep_idx % len(sorted_nodes)] if sorted_nodes else None
            dep_ids = [source.node_id] if source else []
            synthetic.append(
                DAGNode(
                    node_id=f"n{next_id}",
                    tool_name="read_file",
                    raw_args={"filename": {"$artifact": source.node_id, "$field": "file"} if source else "_stress_identity.txt"},
                    dependencies=dep_ids,
                    metadata={"synthetic": True, "noop_identity": True, "stress_index": i},
                )
            )
            next_id += 1
